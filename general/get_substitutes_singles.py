...
# Modified version of get_substitutes.py that works for words with only one mention
...
def main():
    ...
    substitutes_dir = os.path.join(tokenized_dir, subdir, 'subs_masked_singles')
    ...
    instances_in_batches = []
    target_tokens = []
    to_encode = []
    target_word_piece_indices = []

    # Go through all words and put all instances to be encoded in one list
    for term_i, target_term in enumerate(sorted(indices)[start:]):        
        ...
        for index_sample_i, pair in enumerate(pairs):

            line_id = pair[0]
            token_index = pair[1]

            instances_in_batches.append(index_sample_i)
    ...
    print(len(to_encode), 'instances to encode')

    # Now go through all instances and encode them
    batch = 0
    batch_start_index = 0
    while batch_start_index < len(to_encode):
        batch_end_index = min(batch_start_index + batch_size, len(to_encode))
        to_encode_batch = to_encode[batch_start_index:batch_end_index]
        target_word_piece_indices_batch = target_word_piece_indices[batch_start_index:batch_end_index]

        batch_len = len(to_encode_batch)
        if batch_len == 1:
            print("Skipping final batch of length 1")
            batch_start_index += 1
        else:
            running_total += len(to_encode_batch)
            n_rows = len(to_encode_batch)
            
            # get the longest segment, and add two for special tokens
            min_len = min([len(segment) for segment in to_encode_batch]) + 2
            max_len = max([len(segment) for segment in to_encode_batch]) + 2
            print()
            print(batch, n_rows, min_len, max_len, running_total)

            input_ids = np.zeros([batch_len, max_len], dtype=int)
            attention_mask = np.zeros_like(input_ids)

            for s_i, segment in enumerate(to_encode_batch):
            ...
            with torch.no_grad():
                try:
                    ...
                    for row in range(n_rows):
                        target = target_word_piece_indices_batch[row]+1
            ...
            batch += 1
            batch_start_index += batch_len

    print(len(line_ids_to_save), len(target_tokens), len(token_indices_to_save), len(top_words))

    print("Saving")
    outfile = os.path.join(substitutes_dir, 'all_words.jsonlist')
    with open(outfile, 'w') as f:
        for jj, line_id in enumerate(line_ids_to_save):
            f.write(json.dumps({'token': target_tokens[jj],
                                'line_id': line_id,
